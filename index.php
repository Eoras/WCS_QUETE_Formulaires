...
$mail = "/^[\w\-\+]+(\.[\w\-]+)*@[\w\-]+(\.[\w\-]+)*\.[\w\-]{2,4}$/";
    if ($_POST)
    {
// *_*_*_* IF NOT EMPTY *_*_*_* //

        // telephone bien un telephone

        // au moins 6 caractères de long

        $errors = Array();

        // Démarage de la validation des champs vides:

    // *_*_*_* OBLIGATIONS *_*_*_* //
        ...
        {
            $errors['user_phone_empty'] = 'Ecrivez votre numéro comme xx xx xx xx xx';
        }
    ...
    // *_*_*_* VALIDATION SI ERROR = 0 (PAS D'ERREURS) *_*_*_* //
        if(count($errors) == 0)
        {
            session_start();
            $_SESSION['form'] = $_POST;
...
<body>
    <header>
        ...
        <p>Je me suis amusé à faire un peut de REGEX, c'était plutôt amusant, au lieu de faire trop de conditions inutiles. Test mon formulaire et amuse toi. Quand tout sera respecté, tu aura le résumé de ta requête.</p>
        <br/>
    </header>
		...
		<legend>Mon Formulaire</legend>
			...
			<div>
				...
				<select name="user_theme" id="user_theme">
                    <option value=""></option>
					<option value="comedy" <?php if (isset($_POST['user_theme']) && $_POST['user_theme'] == 'comedy') echo 'selected'; ?>>Comedy</option>
					<option value="horror" <?php if (isset($_POST['user_theme']) && $_POST['user_theme'] == 'horror') echo 'selected'; ?>>Horror</option>
					<option value="fantastic" <?php if (isset($_POST['user_theme']) && $_POST['user_theme'] == 'fantastic') echo 'selected'; ?>>Fantastic</option>
				</select>
            ...
            <p class="error"><?php if (isset($errors['user_message_empty'])) echo $errors['user_message_empty']; ?></p><br/>
		    <div class="button">
